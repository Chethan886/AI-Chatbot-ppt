...
// 1. Import the official Google AI provider
import { createGoogleGenerativeAI } from "@ai-sdk/google";
import { NextResponse } from "next/server";
import { z } from "zod";

// 2. Initialize the Google client with your new key
// This key should be from aistudio.google.com
const google = createGoogleGenerativeAI({
  apiKey: process.env.GOOGLE_API_KEY, 
});

// 3. (Your schema - no changes needed)
...
// 4. (Your POST handler - only the model call is changed)
...
    console.log("[v0] Calling Google AI Studio model with generateObject...");
    
    const { object: parsedResponse } = await generateObject({
      // 5. Use the Google client and the correct free model ID
